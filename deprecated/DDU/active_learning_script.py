...
if __name__ == "__main__":
    ...
    if args.ambiguous:
        pretrained_net = models[args.trained_model_name](
            spectral_normalization=args.tsn, mod=args.tmod, mnist=True
        ).to(device)
        pretrained_net = torch.nn.DataParallel(
            pretrained_net, device_ids=range(torch.cuda.device_count())
        )
        cudnn.benchmark = True
        pretrained_net.load_state_dict(
            torch.load(args.saved_model_path + args.saved_model_name)
        )
    ...
    for run in range(num_runs):
        ...
        while True:
            ...
            if args.al_type == "ensemble":
                model_ensemble = [
                    model_fn(
                        spectral_normalization=args.sn, mod=args.mod, mnist=True
                    ).to(device=device)
            ...
            for epoch in range(args.epochs):
                ...
                _, val_accuracy, _, _, _ = (
                    test_classification_net_ensemble(
                        model_ensemble, val_loader, device=device
                    )
                    if args.al_type == "ensemble"
            ...
            if args.al_type == "ensemble":
                ...
                with torch.no_grad():
                    for data, _ in pool_loader:
                        data = data.to(device)
                        mean_output, predictive_entropy, mi = ensemble_forward_pass(
                            model_ensemble, data
                        )
                ...
                if args.al_type == "gmm":
                    ...
                    (
                        candidate_scores,
                        candidate_indices,
                    ) = active_learning.find_acquisition_batch(
            ...
            if args.ambiguous:
                entropies, amb_percent = ambiguous_acquired(
                    small_train_loader, args.threshold, pretrained_net
                )
    ...
    save_name = model_save_name(
        args.model_name, args.sn, args.mod, args.coeff, args.seed
    )
