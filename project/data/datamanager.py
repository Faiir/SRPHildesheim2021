...
class Data_manager():
    ...
    def create_merged_data(self,
        ...
        assert test_size + base_pool_size + labelled_size <= self.base_data.shape[0], \
        ...
        train_data , self.test_data, train_labels, self.test_labels = train_test_split(self.base_data,
                                                                                        self.base_labels,
                                                                                        test_size=test_size,
        ...
        if base_pool_size>0:
            ...
            labelled_data , unlabelled_data, labelled_labels, unlabelled_labels = train_test_split(train_data,
                                                                                                    train_labels,
            ...
            status_list = [np.ones_like(labelled_labels),np.zeros_like(unlabelled_labels)]
            source_list = [np.ones_like(labelled_labels),np.ones_like(unlabelled_labels)]
        ...
        if OOD_ratio>0:
            ...
            assert OOD_size < len(self.OOD_data), f'Insufficient Samples in OOD Dataset : OOD_size > OOD_Dataset : {OOD_size} > {len(self.OOD_data)}'

            OOD_data , _, OOD_labels, _ = train_test_split(self.OOD_data ,
                                                            self.OOD_labels,
                                                            train_size=OOD_size,
        ...
        self.status_manager = pd.DataFrame(np.concatenate([pool_labels[...,np.newaxis],
                                                      pool_source[...,np.newaxis],
                                                      pool_status[...,np.newaxis]],axis=1),columns=['target','source','status'])
        
        self.iter = 0
        
        self.config = {'Total_overall_examples' : len(self.status_manager),
                  'Total_base_examples': len(self.status_manager[self.status_manager['source']>0]),
                  'Total_OOD_examples': len(self.status_manager[self.status_manager['source']<0]),
                  'Initail_examples_labelled':len(self.status_manager[self.status_manager['status']==1])}
    ...
    def get_train_data(self):
        ...
        assert self.iter is not None, 'Dataset not initialized. Call create_merged_data()'
    ...
    def get_unlabelled_pool_data(self):
        ...
        return self.pool_data[unlabelled_mask],self.status_manager.iloc[unlabelled_mask]['target'].values
        
    def add_log(self, log_dict=None):
        self.iter += 1
        current_iter_log = {'Iteration':self.iter,
                            'Base_examples_labelled':len(self.status_manager[self.status_manager['status']>1]),
                            'OOD_examples_labelled':len(self.status_manager[self.status_manager['status']<0]),
                            'Remaining_pool_samples':len(self.status_manager[self.status_manager['status']==0])}
    ...
    def get_logs(self):
        log_df = pd.DataFrame.from_dict(current_data.log,orient='index').set_index('Iteration')
