...
def get_tsne_plot(data_manager, dataset, net, device, plot_ood = True):
    ...
    merged_data  = torch.Tensor(np.concatenate((labelled_data, unlabelled_data))).to(device)
    labels = np.concatenate((labelled_labels, unlabelled_labels))
    source = np.concatenate([np.zeros_like(labelled_labels),map_unlabelled(unlabelled_labels)])
    ...
    with torch.no_grad():
        predictions = net(merged_data.permute(0,3, 1, 2).float())
        tsne_embeddings = TSNE(n_components=2, perplexity=15).fit_transform(predictions.cpu())

    df = pd.DataFrame(np.concatenate([labels[...,np.newaxis], source[...,np.newaxis]],axis=1), columns = ["labels", "source"])
